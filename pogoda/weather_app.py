...
class WeatherApp:
    ...
    # pobieranie danych z API    
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.display_weather_info(data)
                self.display_hourly_forecast(data)
            else:
    ...
    def display_weather_info(self, data): # Wyświetlenie podstawowych danych
        try:
            print(f"Data: {data['days'][0]['datetime']}, Miejsce: {data['resolvedAddress']}")
            print(f"Temperatura: {self.con_and_cons.FtoC(data['days'][0]['tempmax'])}°C, Temperatura Odczuwalna: {self.con_and_cons.FtoC(data['days'][0]['feelslikemax'])}°C")
            print(f"Wiatr: {self.con_and_cons.Mph_to_kmh(data['days'][0]['windspeed'])} km/h {self.con_and_cons.wind_degrees(data['days'][0]['winddir'])}, Opady: {self.con_and_cons.Cal_to_mm(data['days'][0]['precip'])} mm")
            print(f"Wschód słońca: {data['days'][0]['sunrise']}, Zachód słońca: {data['days'][0]['sunset']}")
            print(f"{self.translator.translate(data['days'][0]['description'])}")
    ...
    def display_hourly_forecast(self, data): # Wyświetlanie danych godzinowym
        ...
        try:
            ...
            for i in range(24):
                ...
                print(to_print_1 + to_print_2 + to_print_3 + to_print_4)
        except KeyError as e: print(f"Error: {e}")
        except Exception as e: print(f"Error: {e}")
    ...
    def plot_weather_data(self): # wykresy
        ...
        try:
            ...
            # Wykres temperatury
            bars_temp = temp_series.plot(kind='bar', title='Temperatura [°C]', color='red', ax=ax1, fontsize=8)
            ax1.grid(True)
            ...
            bars_precip = precip_series.plot(kind='bar', title='Szansa Opadów [%]', color='blue', ax=ax2, fontsize=8)
            ax2.set_ylim(0)
            ax2.grid(True)
            ...
            if self.sum_precip > 0:
                bars_pre = pre_series.plot(kind='bar', title='Opady [mm]', color='grey', ax=ax3, fontsize=8)
                ax3.set_ylim(0)
                ax3.grid(True)
...
# country=input("Podaj kraj: ").lower()
city='puńsk'
weather_app = WeatherApp(city)
weather_app.fetch_weather_data()
